<?php include __PUBLIC__ . '/assets/header.php' ?>
  ...
  <ul id="chat"><?php foreach($messages as $message){ $color = ($message['sender_id']==$current_user['id']) ? 'style="color:red;"' : ""; echo '
    <li id="'.$message['id'].'" '.$color.'><img src="'.$message['picture'].'" style="width:50px;" /></br>'.$message['full_name'].': '.$message['body'].'</li>'; } ?>
  </ul>
  ...
  <script>
    (function(global){
      ...
      GROUP_CHAT.append = function(data, current_user){
        var id = data.id;
    ...
    $(function(global){
      ...
      honeybase.current_user(function(flag, current_user){
        pubsub.sub(function(data){
          GROUP_CHAT.append(data.message, data.user);// なんか同期時current_userが投稿したことになっちゃう
        });
